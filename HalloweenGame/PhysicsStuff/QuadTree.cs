...
namespace HalloweenGame.PhysicsStuff
{
    ...
    public class QuadTree<T>
    {
        ...
        public QuadTree(int splitCount, int depthLimit, ref Rectangle2D region)
        {
		...
		/// <param name="depthLimit">Maximum distance a node can be from the tree root.</param>
        /// <param name="region">The region that your quadtree occupies, all inserted quads should fit into this.</param>
		public QuadTree(int splitCount, int depthLimit, Rectangle2D region)
        ...
        public QuadTree(int splitCount, int depthLimit, float x, float y, float width, float height)
            : this(splitCount, depthLimit, new Rectangle2D(x, y, x + width, y + height))
        {
        ...
        public void Insert(T value, ref Rectangle2D quad)
        {
            Leaf leaf;
        ...
        public void Insert(T value, Rectangle2D quad)
        {
        ...
        {
            var quad = new Rectangle2D(x, y, x + width, y + height);
            Insert(value, ref quad);
        }
        ...
        public bool SearchArea(ref Rectangle2D quad, ref List<T> values)
        {
            if (values != null)
                values.Clear();
            else
        ...
        /// <param name="quad">The area to search.</param>
        /// <param name="values">A list to populate with the results. If null, this function will create the list for you.</param>
        public bool SearchArea(Rectangle2D quad, ref List<T> values)
        {
        ...
        static Branch CreateBranch(QuadTree<T> tree, Branch parent, int branchDepth, ref Rectangle2D quad)
        {
            ...
            branch.Quads[0].Set(quad.X, quad.Y, midX, midY);
            branch.Quads[1].Set(midX, quad.Y, quad.Width, midY);
            branch.Quads[2].Set(midX, midY, quad.Width, quad.Height);
            branch.Quads[3].Set(quad.X, midY, midX, quad.Height);
            return branch;
        }

        static Leaf CreateLeaf(T value, ref Rectangle2D quad)
        {
        ...
        internal class Branch
        {
            ...
            internal Rectangle2D[] Quads = new Rectangle2D[4];
            ...
            internal void SearchQuad(ref Rectangle2D quad, List<T> values)
            {
                if (Leaves.Count > 0)
                    for (int i = 0; i < Leaves.Count; ++i)
